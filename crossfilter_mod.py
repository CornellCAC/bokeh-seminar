...
def create_figure():
    ...
    hover_tips = [(c, "@"+c) for c in columns]

    p = figure(height=600, width=800, tools='pan,box_zoom,hover,save,reset',
                   tooltips = hover_tips, **kw)
    ...
    p.circle(x=x.value, y=y.value, source=source, color='color', size='size', line_color="white", alpha=0.6, hover_color='white', hover_alpha=0.5)
